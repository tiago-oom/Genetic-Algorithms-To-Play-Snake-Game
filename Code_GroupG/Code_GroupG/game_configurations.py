...
def starting_positions():
    snake_start = [100, 100]
    snake_position = [[100, 100], [90, 100], [80, 100]]  # going right
...
###########################################################################################################
def isFoodAhead(snake_position, apple_position):
    snake_direction_vector = list(np.subtract(np.array(snake_position[0]), np.array(snake_position[1])))
    snake_position_array = np.array(snake_position[0])
    apple_position_array = np.array(apple_position)

    subtracted_array = np.subtract(apple_position_array, snake_position_array)
    ...
    if snake_direction_vector == subtracted:  # food is ahead
        return True, 0

    elif list(left_direction_vector) == subtracted:  # food is to the left
        return True, -1

    elif list(right_direction_vector) == subtracted:  # food is to the right
        return True, 1
